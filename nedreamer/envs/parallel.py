    ...
    def step(self, action, done):
        ...
        for p, d in zip(promise, done):
            if d:
                ...
                new_r.append(r)
                new_d.append(d)
        obs_stacked = {k: np.stack([o[k] for o in new_o]) for k in new_o[0].keys() if "log_" not in k}
...
class Worker:
    ...
    def __init__(self, fn, strategy="thread", state=False):
        ...
        self.impl = {
            ...
            "thread": bind(DummyWorker, initializers=inits),
        }[strategy](fn)
        self.promise = None
