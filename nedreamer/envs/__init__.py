...
def make_envs(config):
    if config.vectorized:
        ...
        if config.env_num == 1 and config.eval_episode_num == 1:
...
def make_env(config, id):
    ...
    if suite == "dmc":
        ...
        from nedreamer_env_wrapper import make_nedreamer_env
        env = make_nedreamer_env()
        # Add OneHot wrapper since NE-Dreamer wants discrete actions
        env = wrappers.OneHotAction(env)
    else:
